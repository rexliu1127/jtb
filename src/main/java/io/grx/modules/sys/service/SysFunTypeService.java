package io.grx.modules.sys.service;

import io.grx.modules.sys.entity.SysFunTypeEntity;

import java.util.List;
import java.util.Map;

/**
 * 充值类型表
 * 
 * @author randy.huang
 * @email jewian@gmail.com
 * @date 2018-12-13 16:17:36
 */
public interface SysFunTypeService {
	
	SysFunTypeEntity queryObject(Long id);

	List<SysFunTypeEntity> queryList(Map<String, Object> map);
	
	int queryTotal(Map<String, Object> map);
	
	void save(SysFunTypeEntity sysFunType);
	
	void update(SysFunTypeEntity sysFunType);
	
	void delete(Long id);
	
	void deleteBatch(Long[] ids);

	int queryCountByConditions(Map<String, Object> map);

	SysFunTypeEntity queryObjectByConditions(Map<String, Object> map);

	void saveFunTypeList(List<SysFunTypeEntity> sysFunTypeList);

	List<SysFunTypeEntity>  queryInfo(String merchantNo);
}
