package io.grx.modules.sys.dao;

import java.util.List;
import java.util.Map;

import org.apache.ibatis.annotations.Mapper;
import org.apache.ibatis.annotations.Param;

import io.grx.modules.sys.entity.SysUserEntity;

/**
 * 系统用户
 *
 * @author chenshun
 * @email sunlightcs@gmail.com
 * @date 2016年9月18日 上午9:34:11
 */
@Mapper
public interface SysUserDao extends BaseDao<SysUserEntity> {

	/**
	 * 查询用户的所有权限
	 * @param userId  用户ID
	 */
	List<String> queryAllPerms(Long userId);

	/**
	 * 查询用户的所有菜单ID
	 */
	List<Long> queryAllMenuId(Long userId);

	/**
	 * 根据用户名，查询系统用户
	 */
	SysUserEntity queryByUserName(@Param("username") String username);

	/**
	 * 修改密码
	 */
	int updatePassword(Map<String, Object> map);

	/**
	 * 根据email，查询系统用户
	 */
	SysUserEntity queryByEmail(@Param("email") String email);

	List<SysUserEntity> queryFreeAuditorList();

	boolean isDeptHasUser(Long deptId);

	List<SysUserEntity> querySpecifiedFreeAuditorList(@Param("channelId") Long channelId);
}
