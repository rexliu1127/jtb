...
$(function(){
    ...
    function reloadData() {
        ...
        $.showLoading('加载中...');
        $.get(baseURL + "rcpt/borrowed_transaction/list?lenderName="+searchValue, function(r){
            $.hideLoading();
            if (r.page.length === 0) {
                emptyDiv.show();
            } else {
                ...
                r.page.forEach(function(e) {
                    var tr = '<li class="display" onclick="location.href=\'transaction/' + e.txId + '.html\'"><div' +
                        ...
                        ' src="' + (e.lenderHeadImg ? e.lenderHeadImg : '../img/accoubt/photo.svg') + '"></div><div class="borrowers flex1">' +
                        '<h3>' + e.lenderName + ' <i class="radius7PX">出借人</i></h3><span>开始日期：' + e.beginDate.substring(0, 10)
