...
function openRepayment(txId) {
    location.href = baseURL + 'rcpt/repayment_repay_plan/' + txId + pageExt;
}
...
var loadData = function(searchValue, page, limit) {
    $.showLoading('加载中...');
    $.get(baseURL + "rcpt/repayment/repay_list?userName="+searchValue + "&page=" + page + "&limit=" + limit, function(r){
        $.hideLoading();
        if (r.page.length === 0) {
            noMore = true;
